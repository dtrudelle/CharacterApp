...
struct CharacterSheetView: View {
    ...
    func pageViews() -> [AnyView] {
        [AnyView(page("page 1 / 3") { page1 }),
         AnyView(page("page 2 / 3") { page2 }),
         AnyView(page("page 3 / 3") { page3 })]
    }
    ...
    private var page1: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(alignment: .top, spacing: 18) {
                savesColumn.frame(width: 150, alignment: .leading)
                skillsColumns
            }
            healthBand
        ...
        VStack(spacing: 0) {
            ForEach(lines, id: \.skill) { line in
                HStack(spacing: 7) {
                    ...
                    // Caractéristique régissante (abréviation anglaise SRD).
                    Text("(\(line.skill.ability.rawValue))")
    ...
    private var combatBlock: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Grid(horizontalSpacing: 10, verticalSpacing: 8) {
                GridRow {
                    metric("CA", "\(sheet.armorClassComputed)")
                    metric("Initiative", fmt(sheet.initiative))
                }
                GridRow {
    ...
    // MARK: - Page 2 (descriptif : sorts, outils, équipement, monnaie, notes)

    private var page2: some View {
        VStack(alignment: .leading, spacing: 16) {
            if !sheet.knownSpells.isEmpty {
    ...
    // MARK: - Page 3 (capacités & traits, en deux colonnes)

    /// Groupes de capacités liés à la classe et à la sous-classe (colonne gauche).
    private var classSubclassGroups: [ComputedSheet.FeatureGroup] {
        sheet.featureGroups.filter { $0.origin == .characterClass || $0.origin == .subclass }
    }

    /// Groupes de capacités liés à l'espèce (colonne droite, avec les feats).
    private var speciesGroups: [ComputedSheet.FeatureGroup] {
        sheet.featureGroups.filter { $0.origin == .species }
    }

    private var page3: some View {
        HStack(alignment: .top, spacing: 22) {
            // Colonne gauche : classe & sous-classe.
            VStack(alignment: .leading, spacing: 14) {
                sectionLabel("Classe & sous-classe")
                ForEach(Array(classSubclassGroups.enumerated()), id: \.offset) { _, group in
                    featureGroupBlock(group)
                }
                if classSubclassGroups.isEmpty {
                    Text("—").font(.callout).foregroundStyle(.secondary)
                }
            }
            .frame(maxWidth: .infinity, alignment: .leading)

            // Colonne droite : espèce & feats.
            VStack(alignment: .leading, spacing: 14) {
                sectionLabel("Espèce & feats")
                ForEach(Array(speciesGroups.enumerated()), id: \.offset) { _, group in
                    featureGroupBlock(group)
                }
                if !sheet.feats.isEmpty {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Feats").font(.caption.weight(.medium)).foregroundStyle(.secondary)
                        ForEach(sheet.feats) { feat in
                            featureRow(name: feat.name, tag: feat.category.rawValue, description: feat.shortEffect)
                        }
                    }
                }
                if speciesGroups.isEmpty && sheet.feats.isEmpty {
                    Text("—").font(.callout).foregroundStyle(.secondary)
                }
            }
            .frame(maxWidth: .infinity, alignment: .leading)
    ...
    /// Un groupe de capacités : en-tête de provenance + une ligne par capacité.
    private func featureGroupBlock(_ group: ComputedSheet.FeatureGroup) -> some View {
        VStack(alignment: .leading, spacing: 6) {
    ...
    /// Une case vide par dé de vie (fiche papier : cochée au crayon), par rangées
    /// de `perRow` (10 par défaut pour les dés de vie ; 4 pour les points de focus,
    /// qui dépassent rarement 4 et resteraient sinon sur une seule ligne trop large).
    private func dieBoxes(total: Int, perRow: Int = 10) -> some View {
        let step = max(1, perRow)
        let rows = stride(from: 0, to: max(total, 0), by: step).map { Array($0..<min($0 + step, total)) }
