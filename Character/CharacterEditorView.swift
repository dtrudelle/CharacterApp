...
struct LevelASIState {
    ...
    var a2: Ability = .DEX
    var featId: String = ""
    /// Caractéristique choisie pour le +1 d'un don qui en accorde un.
    var featAbility: Ability = .STR
...
struct BonusFeatEntry: Identifiable { let id = UUID(); var featId: String; var ability: Ability = .STR }
...
struct CharacterEditorView: View {
    ...
    init(character: Character, library: ContentLibrary, onChange: @escaping (Character) -> Void) {
        ...
        for fc in character.featChoices {
            if let lvl = Self.level(from: fc.source) {
                ...
                st.kind = .feat
                st.featId = fc.featId
                st.featAbility = fc.abilityBonus ?? .STR
                levels[lvl] = st
            }
        }
        ...
        _bonusFeatIds = State(initialValue: character.featChoices
            ...
            .map { BonusFeatEntry(featId: $0.featId, ability: $0.abilityBonus ?? .STR) })
    ...
    private var healthCombatSection: some View {
        Section("Santé & combat") {
            let bases = combatBases
            combatStat("Points de vie", base: bases.hitPoints,
                       bonus: $character.hitPointsBonus, signed: false)
            combatStat("Classe d'armure", base: bases.armorClass,
                       bonus: $character.armorClassBonus, signed: false)
            combatStat("Initiative", base: bases.initiative,
                       bonus: $character.initiativeBonus, signed: true)
        }
    }

    /// Une ligne « calcul auto + ajustement manuel ».
    /// Affiche le total à droite ; sous le titre, le détail « base + bonus »
    /// uniquement quand le bonus est non nul. Les deux flèches éditent le bonus.
    @ViewBuilder
    private func combatStat(_ title: String, base: Int, bonus: Binding<Int>, signed: Bool) -> some View {
        let b = bonus.wrappedValue
        let total = base + b
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                Text(title)
                if b != 0 {
                    Text("\(signed ? signedInt(base) : "\(base)") \(b >= 0 ? "+" : "−") \(abs(b))")
                        .font(.caption).foregroundStyle(.secondary).monospacedDigit()
                }
            }
            Spacer()
            Text(signed ? signedInt(total) : "\(total)")
                .font(.body.weight(.medium)).monospacedDigit()
            Stepper("", value: bonus).labelsHidden()
        }
    }

    /// Bases de combat AUTO (sans bonus) du personnage en cours d'édition.
    /// Si la sélection est incomplète (pas de classe/espèce/historique), repli sur
    /// des valeurs neutres dérivées des seules caractéristiques.
    private var combatBases: SheetBuilder.CombatBases {
        if let refs = library.references(for: liveCharacter) {
            return SheetBuilder.combatBases(liveCharacter, refs)
        }
        let ab = SheetBuilder.finalAbilities(liveCharacter)
        return .init(hitPoints: 0, armorClass: 10 + ab.modifier(.DEX), initiative: ab.modifier(.DEX))
    }

    private func signedInt(_ n: Int) -> String { n >= 0 ? "+\(n)" : "\(n)" }
    ...
    private var equipmentSection: some View {
        Section("Équipement") {
            // Pièces (monnaie de campagne Thalaris).
            ...
            // Menus de sélection des catalogues.
            equipmentPicker("Armes", key: "weapons",
                            items: library.weapons, ids: \.ownedWeaponIds)
            equipmentPicker("Armures", key: "armor",
                            items: library.armor, ids: \.ownedArmorIds)
            equipmentPicker("Outils", key: "tools",
                            items: toolsAsEquipment, ids: \.ownedToolIds)
            equipmentPicker("Équipement général", key: "gear",
                            items: library.gear, ids: \.ownedGearIds)
            // Objets hors catalogue, en texte libre.
            LabeledContent("Autre (texte libre)") { EmptyView() }
    ...
    /// Bloc de sélection d'équipement (sous-en-tête + recherche + DisclosureGroup
    /// par `group` + Toggle par item). Sans `Section` : destiné à être composé
    /// dans la section « Équipement ».
    @ViewBuilder
    private func equipmentPicker(
    ...
    private func levelASIRow(_ lvl: Int) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            case .feat:
                ...
                featAbilityPicker(featId: b.wrappedValue.featId, selection: b.featAbility)
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            ForEach($bonusFeatIds) { $entry in
                HStack {
                    ...
                    .labelsHidden()
                    featAbilityPicker(featId: entry.featId, selection: $entry.ability)
                    Button(role: .destructive) {
    ...
    /// Caractéristiques éligibles au +1 du don (vide si le don n'en accorde pas).
    private func featAbilityOptions(_ featId: String) -> [Ability] {
        library.feats.first { $0.id == featId }?.abilityBonusOptions ?? []
    }

    /// Picker « +1 [carac] » affiché seulement si le don accorde un bonus.
    /// Le binding est borné aux options du don (repli sur la première si invalide).
    @ViewBuilder
    private func featAbilityPicker(featId: String, selection: Binding<Ability>) -> some View {
        let options = featAbilityOptions(featId)
        if !options.isEmpty {
            let clamped = Binding<Ability>(
                get: { options.contains(selection.wrappedValue) ? selection.wrappedValue : options[0] },
                set: { selection.wrappedValue = $0 }
            )
            HStack(spacing: 4) {
                Text("+1").font(.caption).foregroundStyle(.secondary)
                Picker("", selection: clamped) {
    ...
    /// Valeur à persister pour le +1 d'un don : nil si le don n'en accorde pas,
    /// sinon la carac choisie (bornée à ses options).
    private func featAbilityBonus(featId: String, chosen: Ability) -> Ability? {
        let options = featAbilityOptions(featId)
        guard !options.isEmpty else { return nil }
        return options.contains(chosen) ? chosen : options[0]
    }
    ...
    private var assembledFeatChoices: [FeatChoice] {
        let levelFeats = asiLevelsUpTo.compactMap { lvl -> FeatChoice? in
            ...
            return FeatChoice(source: "Level \(lvl)", featId: st.featId,
                              abilityBonus: featAbilityBonus(featId: st.featId, chosen: st.featAbility))
        }
        let bonus = bonusFeatIds.filter { !$0.featId.isEmpty }
            .map { FeatChoice(source: "Bonus", featId: $0.featId,
                              abilityBonus: featAbilityBonus(featId: $0.featId, chosen: $0.ability)) }
